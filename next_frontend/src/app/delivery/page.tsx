...
const STATUS_COLORS: Record<string, string> = {
  ...
  pending:    "bg-slate-600/20 text-slate-300 border-slate-600/30",
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-100">Delivery Dashboard</h1>
          <p className="text-sm text-slate-400 mt-0.5">Welcome, <span className="text-orange-300">{userName ?? "Rider"}</span></p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
        {[
          { label: "Total Assigned", value: total,     icon: "📦", color: "text-slate-100" },
          { label: "Delivered",      value: delivered,  icon: "✅", color: "text-emerald-300" },
      ...
      {msg && <p className="rounded-xl bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300 border border-emerald-500/20">{msg}</p>}

      {/* Orders list */}
      <div>
        <h2 className="text-sm uppercase tracking-widest text-slate-400 mb-3">Assigned Deliveries</h2>
        {loading ? (
          <p className="text-slate-400 text-sm">Loading…</p>
        ) : orders.length === 0 ? (
          <div className="rounded-2xl border border-slate-700/60 bg-slate-900/60 p-10 text-center">
            <p className="text-3xl mb-2">🛵</p>
            <p className="text-slate-300 font-medium">No deliveries assigned yet</p>
          ...
          <div className="space-y-3">
            {orders.map(order => {
              ...
              return (
                <div key={order.id} className="rounded-2xl border border-slate-700/60 bg-slate-900/60 p-4 flex flex-col sm:flex-row sm:items-center gap-4">
                  <img src={order.product_image} alt={order.product_name} className="h-14 w-14 rounded-xl object-cover flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-slate-100 truncate">{order.product_name}</p>
                    <p className="text-xs text-slate-400">Qty: {order.quantity} · {asCurrency(Number(order.price) * order.quantity)}</p>
                    ...
                    {!isDone && (
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => updateStatus(order.id, "delivered")} disabled={isBusy}
                          className="rounded-full bg-emerald-400 px-3 py-1 text-xs font-bold text-slate-950 hover:bg-emerald-300 transition disabled:opacity-50">
                          {isBusy ? "…" : "✅ Delivered"}
                        </button>
                      </div>
                    )}
                    {isDone && <span className="text-xs text-emerald-500 font-semibold">🎉 Completed</span>}
                  </div>
                </div>
              );
            })}
